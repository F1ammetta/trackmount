...
char *data = "Hello, this is Sidus Mount!\n";
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param) {
  ...
  case ESP_SPP_CONG_EVT:
    ...
    if (!param->cong.cong && connected) {
      ...
      esp_spp_write(param->cong.handle, strlen(gps_data_str),
                    (uint8_t *)gps_data_str);
    }
    break;
...
void app_main(void) {
  gps_data_str = (char *)malloc(128);
  // innitialize the uart
  ...
  // set io pins for uart
  // tx 1 and rx 3
  ESP_ERROR_CHECK(
      uart_set_pin(uart_num, 19, 18, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
