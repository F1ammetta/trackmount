...
#define bmx280_verify(chip_id)                                                 \
  (((chip_id) == BME280_ID) || ((chip_id) == BMP280_ID2) ||                    \
   ((chip_id) == BMP280_ID1) || ((chip_id) == BMP280_ID0))

/**
 * Returns false if the sensor was not found.
 * @param bmx280 The driver structure.
 */
#define bmx280_validate(bmx280)                                                \
  (!(bmx280->slave == 0xDE && bmx280->chip_id == 0xAD))
 ...
 * @returns Error codes.
 */
static esp_err_t bmx280_read(bmx280_t *bmx280, uint8_t addr, uint8_t *dout,
                             size_t size) {
  esp_err_t err;
...
static esp_err_t bmx280_write(bmx280_t *bmx280, uint8_t addr,
...
static esp_err_t bmx280_probe_address(bmx280_t *bmx280) {
  esp_err_t err = bmx280_read(bmx280, BMX280_REG_CHPID, &bmx280->chip_id,
                              sizeof bmx280->chip_id);
  ...
  if (err == ESP_OK) {
    if (
        ...
        bmx280->chip_id == BMP280_ID2 || bmx280->chip_id == BMP280_ID1 ||
        bmx280->chip_id == BMP280_ID0
#else
        bmx280_verify(bmx280->chip_id)
#endif
    ) {
      ESP_LOGI("bmx280", "Probe success: address=%hhx, id=%hhx", bmx280->slave,
               bmx280->chip_id);
      return ESP_OK;
    } else {
  ...
  ESP_LOGW("bmx280", "Probe failure: address=%hhx, id=%hhx, reason=%s",
           bmx280->slave, bmx280->chip_id, esp_err_to_name(err));
  return err;
}

static esp_err_t bmx280_probe(bmx280_t *bmx280) {
  ESP_LOGI("bmx280", "Probing for BMP280/BME280 sensors on I2C %d",
           bmx280->i2c_port);
...
void bmx280_close(bmx280_t *bmx280) { free(bmx280); }
...
esp_err_t bmx280_configure(bmx280_t *bmx280, bmx280_config_t *cfg) {
  ...
  uint8_t num =
      (cfg->t_sampling << 5) | (cfg->p_sampling << 2) | BMX280_MODE_SLEEP;
...
int32_t BME280_compensate_T_int32(bmx280_t *bmx280, int32_t adc_T) {
  int32_t var1, var2, T;
  var1 = ((((adc_T >> 3) - ((int32_t)bmx280->cmps.T1 << 1))) *
          ((int32_t)bmx280->cmps.T2)) >>
         11;
  var2 = (((((adc_T >> 4) - ((int32_t)bmx280->cmps.T1)) *
            ((adc_T >> 4) - ((int32_t)bmx280->cmps.T1))) >>
           12) *
...
uint32_t BME280_compensate_P_int64(bmx280_t *bmx280, int32_t adc_P) {
  ...
  var1 = ((var1 * var1 * (int64_t)bmx280->cmps.P3) >> 8) +
         ((var1 * (int64_t)bmx280->cmps.P2) << 12);
...
esp_err_t bmx280_readout(bmx280_t *bmx280, int32_t *temperature,
  ...
  if (temperature) {
    ...
    *temperature = BME280_compensate_T_int32(
  ...
  if (pressure) {
    ...
    *pressure = BME280_compensate_P_int64(
        bmx280, (buffer[0] << 12) | (buffer[1] << 4) | (buffer[0] >> 4));
  }
  ...
  {
    if (humidity) {
      if ((error = bmx280_read(bmx280, BMX280_REG_HUMI_MSB, buffer, 2)) !=
          ESP_OK)
        return error;

      *humidity =
          bme280_compensate_H_int32(bmx280, (buffer[0] << 8) | buffer[0]);
    }
  }
#if CONFIG_BMX280_EXPECT_DETECT
  else if (humidity)
